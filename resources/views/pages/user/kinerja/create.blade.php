...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('user.kinerja.store') }}" method="POST">
                @csrf
                ...
                <div class="row">
                      <div class="col-md-12 mb-3">
                        ...
                        <select name="tugas_id" class="select2 form-control bg-white" required>
                                ...
                                @foreach ($tugas as $item)
                                    <option value="{{ $item->id }}" {{ old('tugas_id') == $item->id ? 'selected' : '' }}>{{ $item->tugas }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea name="aktifitas" rows="3" placeholder="Masukkan Aktifitas anda hari ini" type="text" class="form-control @error('aktifitas') is-invalid @enderror" required>{{ old('aktifitas') }}</textarea>
                        @error('aktifitas')
